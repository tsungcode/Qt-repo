    ...
    , ui(new Ui::QtSendFileServe)
{
    ...
    connect(this->m_tcp,&QTcpServer::newConnection,this,[=]()
    {
        ...
        //创建子线程对象
        RecvFile* subThread=new RecvFile(tcp);  //将套接字对象通过构造传给子线程
        subThread->start();  //子线程启动调用Run方法

        //主线程接收到子线程接收完数据信号，主线程进行资源释放
...
void QtSendFileServe::on_m_setListen_clicked()
{
    ...
    this->m_tcp->listen(QHostAddress::Any,port); //服务器启动监听
}
